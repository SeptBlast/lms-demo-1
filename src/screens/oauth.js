...
import zuz from "../functions/functions";
...
function AccountPage(props) {
    ...
    useEffect(() => {
        if (!window.recaptchaverifier) {
            window.recaptchaverifier = new global.firebase.auth.RecaptchaVerifier(
                "__phonesigner__"
            );
                ...
                .catch((err) => {
                    ...
                    switch (err.code) {
                        case "auth/invalid-phone-number":
                            zuz.Toast.show({
                                html:
                                    "That number is not valid. Please provide a valid phone number...",
                            });
                            break;
                        default:
                            zuz.Toast.show({
    ...
    const verifyCodeView = () => {
        return (
            <div className="oauth-view oauth-view-verify rel">
                ...
                <h1 className="s18 oline fontn">
                    Enter verifcation code sent to{" "}
                    <span className="fontb">{dialCode + phone}</span>
                </h1>
                ...
                <button
                    ...
                    className="button s24 fontb cfff"
                >
                    Continue
                </button>
            </div>
    ...
    const sendCodeView = () => {
        return (
            <div className="oauth-view rel">
                ...
                <h1 className="s18 oline fontn">
                    Enter your phone number and we will send one-time
                    verifcation code
                </h1>
    ...
    return mod === "sendcode" ? sendCodeView() : verifyCodeView();
}
